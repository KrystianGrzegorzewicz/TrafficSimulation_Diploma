...
)
{
	...
	if (perception.hasCarAhead)
	{
		...
		float desiredGap = 2.5f + s0 + v * T + (v * dv) / (2.0f * std::sqrt(maxAccel * maxDecel + 0.001f));
		float ratio = desiredGap / s;
		interaction = ratio * ratio;
	}

	float accel = (maxAccel * accelFactor) * (1.0f - freeRoad - interaction);
	accel = std::clamp(accel, -maxDecel, maxAccel);

	return accel;
}

void IDMLongitudinalModel::setDriverFactors(float gapFactor, float accelFactor)
{
